...
exports.createSize = async (req, res) => {
  try {
    ...
    if (!name || !code)
      return res
        .status(400)
        .json({ success: false, error: "Tên và mã size là bắt buộc!" });

    const nameExists = await Size.findOne({ name });
    if (nameExists)
      return res
        .status(400)
        .json({ success: false, error: "Tên size đã tồn tại!" });

    const codeExists = await Size.findOne({ code });
    if (codeExists)
      return res
        .status(400)
        .json({ success: false, error: "Mã size đã tồn tại!" });

    const size = new Size({ name, code, isActive: true });
    await size.save();
    res
      .status(201)
      .json({ success: true, message: "Thêm size thành công!", size });
  } catch (error) {
...
exports.updateSize = async (req, res) => {
  try {
    ...
    if (!size)
      return res
        .status(404)
        .json({ success: false, error: "Size không tồn tại!" });
